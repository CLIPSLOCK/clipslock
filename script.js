...
async function sendOrder(order) {
    ...
    try {
        ...
        if (response.ok && result.ok) {
            alert(`Дякуємо! Ваше замовлення ${order.orderId} успішно оформлено.`);
            cart = [];
            saveCart();
        ...
        alert('Не вдалося відправити замовлення. Будь ласка, спробуйте ще раз або зв’яжіться з нами через Telegram/Viber.');
    } finally {
        if (submitBtn) {
            submitBtn.textContent = originalBtnText || 'Підтвердити замовлення';
